...
vignettes/           # 6 vignettes (Getting Started, IRT, LCA/LRA, Biclustering, BNM/LDLRA, GRM)
docs/                # .gitignore'd; pkgdown site deployed to gh-pages branch via GitHub Actions
```
...
- **CI/CD**: GitHub Actions — `R-CMD-check.yaml` (multi-platform), `test-coverage.yaml`
  (codecov), `pkgdown.yaml` (site deployment to gh-pages), `rhub.yaml`
...
### v1.9.0 (ready for CRAN submission)
- ~~CAIC formula correction~~ — Done (commit 9f3b57e)
- ~~Test suite modernization~~ — Done (Excel→CSV, tidyverse/readxl removed)
