...
>
> **注意**: CRANビルド時間短縮のため、計算負荷の高いコード例は `eval=FALSE` としています。完全な実行結果は [pkgdownサイト](https://kosugitti.github.io/exametrika/articles/guide-ja.html) をご覧ください。
...
```{r model-irt, eval=FALSE}
result.IRT <- IRT(J15S500, model = 3)
result.IRT
```

```{r results-irt-ability, eval=FALSE}
head(result.IRT$ability)
```

```{r plot-irt-curves, eval=FALSE}
...
```{r model-lca, eval=FALSE}
result.LCA <- LCA(J15S500, ncls = 5)
head(result.LCA$Students)
```

```{r plot-lca, eval=FALSE}
...
```{r model-lra, eval=FALSE}
result.LRA <- LRA(J15S500, nrank = 6)
head(result.LRA$Students)
```

```{r plot-lra, eval=FALSE}
...
```{r model-biclustering, eval=FALSE}
Biclustering(J35S515, nfld = 5, ncls = 6, method = "B")
```

```{r model-ranklustering, eval=FALSE}
