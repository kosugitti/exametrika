...
`develop/Algorithm_LRA.tex` をユーザ自身が書き直した。コード側は命名の微修正のみ。

### コード(R/00_isotonic_CORE.R)

- `iso_surv()` → **`iso_upper_cum()`** にリネーム。中身も docstring も上側累積
  $S_{cb}=P(\ge b+1)$ であり、生存関数ではない(ユーザ指摘「生存という語は使わない」)。
  呼び出し元は `16_Biclustering_ordinal.R:273` と `test-isotonic-core.R:33`。内部関数
  (`@noRd`)なのでAPI影響なし。NEWS.mdにInternalセクションを追加。
- `iso_build_pi()` 内の `tl`/`tu` → **`theta_lower`/`theta_upper`**。threshold ではなく
  「自分が下側/上側の当事者である対の乗数」の意。
- 4関数すべてに明示的 `return()`(ユーザが自ら加筆。以後Rコードは暗黙の戻り値を書かない
...
- **記号**: 一度カテゴリ番号 $q$ と「切れ目番号 $b$」を分ける案を実装したが、ユーザの
  手描き図(IMG_0627)で「$b$ の位置はここか?」と破綻が露呈し**全面撤回**。$\pi^\ast_{jcq}=P(\ge q)$
  はカテゴリ番号 $q$ で添字づけるのが自然で、切れ目という別対象を立てる必要はなかった。
  和のダミーは $k$。「閾値」の語もGRMの困難度と紛れるため排除。
- **$\theta$ の添字順**をユーザ判断で $\theta_{jcq}$(項目・**隣接ランク対**・カテゴリ)に統一。
  第2添字がランクでなく対の番号($1..C-1$)である点が最大の誤読ポイントだったので本文に明記。
- **展開を省略しない**: $T=\sum\theta(\pi^\ast_c-\pi^\ast_{c+1})$ から
  $d_{jcq}=\sum_{k=2}^q(\theta_{jck}-\theta_{jc(k-1)})$ までを、(1)$\pi^\ast$を$\pi$に開く
  (2)$c'=c+1$で添字を揃え境界$\theta_{jc0}=\theta_{jCq}=0$を使う (3)$q$と$k$の和を入れ替える
  (4)$d$と命名、の4段で明示。(3)がforループの入れ替えである旨をユーザが\verb|for|文で表現。
- **ラグランジュ乗数の説明を追加**(ユーザ「ラグランジュ乗数がわかってない」)。等式制約の
  $\lambda_{jc}$(符号自由・影の価格)と不等式制約の $\theta_{jcq}\ge0$(片側の壁・相補スラック)
  を区別。$\theta<0$ は存在しない($\theta\ge0$がKKTの一部。逆転は解に残らず必ずタイになる)。
- **$\lambda$ の二分探索**を手順箱に。上限は $\sum_q\gamma$ を試し $f>1$ なら倍々に広げる
  ($d$に大きな負成分があると $\sum\gamma$ が下限 $-\min d$ を下回り区間が空になるため。
  実装が倍々にしている理由)。
- **数値例**($C=2,Q=3$、度数 (5,25,20)/(20,5,25))を新設し、$v=0.75$、$\lambda=(20,80)$、
  $\theta_{j12}=40$ を手計算で追える形に。$\lambda_1+\lambda_2$ を2通りに書いて $\theta$ を
  消去する解き方。**タイの水準が度数プール値 $(45+30)/(50+50)$ に一致する**のは、制約1本なら
  多項尤度が二項部分と条件付き多項部分に分解し制約が二項部分にしか触れないため。
- **図2枚を新規作成**:
  - `develop/fig_isotonic_terrace.tex` — $C\times Q$格子上の階段面。ランク方向は登り($\theta$で
    課す)、カテゴリ方向は下り(上側累積の定義から自動)。$\pi^\ast_{jc1}=1$は全面を覆う天板。
    ユーザの手描き(IMG_0628/0629)に合わせ**最小値が手前**に来る投影に変更。
  - `develop/fig_isotonic_pool.tex` — 上の地形のランク軸断面。押し込み前後2枚。数値例に対応。
  - 注釈ノードは格子座標でなく画面座標で直接置く(重なり調整のため)。TikZの`at \PT{}{}{}`は
    パーサが`(`を要求するので `\path \PT{...} node[...]` と書く必要がある(ハマりどころ)。
- **引用をjpa2化**。`~/Dropbox/myBiber.bib` に elbarmi1994 / dykstra1983 / barlow1972 /
  robertson1988 / rockafellar1970 を追加(ayer1955は既存)。`develop/myBiber.bib`はDropbox
  実体へのsymlink。DOIのはみ出しも解消。**Fenchel双対の出典はDykstraではなくFenchel(1949)**、
  整備はRockafellar(1970)、多項尤度への応用がEl Barmi & Dykstra(1994)という三層。
- `\procbox`(fbox+minipage)を`tcolorbox`環境に置換。

### 判明した誤り(ユーザの理解と私の説明の両方)

- 私の「$\theta$ はカテゴリ方向に一切干渉しない」は**誤り**。$\theta$ はカテゴリ方向の値を
  動かす($q$以上の分母にだけ入り、行和1のため配分が変わる)。保たれるのは**単調性**のほうで、
  それは $\pi>0$(有理形で分子正・分母正)から従い $\theta$ に依存しない。数値例でも
  $\theta_{j13}=0$なのに $q=3$ の上側累積が $0.40\to0.3333$、$0.50\to0.6250$ と動く
  ($\lambda$ 経由)。閾値ごとPAVAはここを動かさないので分岐し、この例で $\Delta ll=2.83$。
- ユーザ手描き図(IMG_0632)でランク方向の段差に $\theta$ とラベルされていたのは逆。段差が
  見えている=余裕あり=$\theta=0$、平ら=$\theta>0$(相補スラック)。

### 検算スクリプト

`scratchpad/ex23.R`(未移動)。iso_dual_map vs 閾値ごとPAVAをloglikで比較。完全逆転例では
両者一致、$q=2$のみ違反の例で $\Delta ll=2.83$。develop/へ移すかは未判断。

**次**: (a)メモの通読は完了(誤植・記号不統一10件を指摘しユーザが修正済)、(b)A3本文へ
...
- **GridSearch: 対応済み・手当て不要**。`...` を `do.call(fun, args_list)` で流すだけの
  ラッパーで、`estimation="isotonic"` はそのまま届く。実測確認
  (`GridSearch(J35S515, max_ncls=4, max_nfld=3, fun="Biclustering", method="R",
  estimation="isotonic")` → `optimal_result$estimation == "isotonic"`)。
- **IRM: 未対応。ただし設計上そもそも軸が無い**。`Biclustering_IRM.*` に `method` 引数は
  無く、バイクラ側の `method="B"/"R"`(無順序/順序)に相当する対が存在しない。18番の
  M-stepは今も無制約の累積正規化(`rev(cumsum(rev(Ufcq_prior[f,cc,])))`)のまま。
- **18番の `mic` は16番と同型の飾り**。全フィールド合計期待得点でクラスを昇順に
  並べ替えるだけで、SOACは課さず推定値も変わらない。roxygenが「forces FRP to be
  monotonically increasing」と実態に反していたので実態に合わせて書き直した(既定FALSEなので
...
**今のIRMに順序が入らない理由は2つ重なっている**: (1)崩壊型Gibbsが壊れる。πを積分消去
...
捨てる=事前分布の取り替え。**非対称性が鍵**: フィールドに順序は無い(CRPが正しい)。
...
**ユーザ判断: 「案Bがいる、案Cがいけてもいずれいる」。将来のexametrika機能。**
memory: project_exametrika_future 項目6 に設計を記録。

### 検証: 案Cは成立するか(J35S500 ordinal isotonic, nfld=5)

クラス数を振って「実効水準数が頭打ちになるか」を見た。**結論: 案Cはランク数の推定法に
ならない。診断としても成立しない。**

第1回(完全併合で数える)。実効水準数が指定クラス数と完全に一致(4→4, 6→6, 8→8, 10→10,
12→12)。フィールド別も同じ。**完全併合はゼロ**。ただしこの測り方は厳しすぎた。制約は
(フィールド, 隣接クラス対, 閾値)の1つずつに立つのでアクティブになるのは特定の閾値だけで、
プロファイル全体の一致は起きにくい事象。
...
**副産物(A3に使える)**: 「アクティブ率」は順序制約がどれだけ働いているかの指標として
...
案Cの結論も不変。**ただし2点気になる**: (1)反復数が条件で極端に違う(5,7,125,26,6)。
BIC最小のncls=6だけ265秒と桁違いに遅い=平坦な領域か境界近くの解。(2)**loglikがクラス数に
ついて単調でない**(6:−20602.0 → 7:−20603.1)。原理的には増えるはずなのでEMの局所解。差は1.1
と小さいが尤度面が凸凹している証拠。

**A3の運用ルールにすべきこと**: (a)`maxiter`既定100は足りない。実データはもっと大きいので
余裕を見る。収束フラグを必ず確認し非収束なら採用しない。(b)局所解があるので初期値を変えた
複数回実行が要る。1回の実行のBICでクラス数を選ぶのは危険。

**Rの落とし穴(2回踏んだ)**: `TestFitIndices` の列名は `BIC`/`AIC`/`CAIC` で
`model_BIC` は存在しない。存在しない要素を `sprintf` に渡すと長さ0→`character(0)` が
返り、`cat` は何も出力せず**エラーも警告も出ない**。表のヘッダだけ出て中身が空になる。
検算スクリプトでは `flush(stdout())` と併せて要注意。

スクリプト: `scratchpad/eff_rank.R`(完全併合版) / `eff_rank2.R`(閾値レベル版) /
...
**私の当初の懸念は的外れだった**: 「A3の一撃(MSPS F=4復元)は7/19に直した配線バグの影響下
...
**より大きな論点が2つ見つかった**:
1. **用語**: 本文はIRMの出力を「18潜在ランク」と呼びアレイ図から順序勾配を読んでいるが、
   IRMのクラスはCRPで交換可能=順序を持たない。並んで見えるのは並べ替えの結果。査読で
   「これはランクか、順序を持たないクラスを事後に並べただけか」と問われうる。今日の議論
   (設計格子の右下が空白)がこの弱点の理論的裏づけになる。
2. **§2.3の記述**: 「分子が減少するので π*_{fc1}≥…≥π*_{fcQ} が自動的に成立。この構造的
...
**次**: (a)A3原稿の図表・数値を「IRMなので影響なし/ランクラスタリングなので再実行/記述の
...
改善されるか、確認してみたい」→ 試作で桁違いの改善が出たのでそのまま本体へ組み込み。

### なぜ効いたか

`iso_dual_map` は**二分探索が二重**になっている。外側は各 θ を制約がタイになるまで上げる
探索、内側は行和1を満たす λ の探索。しかも内側は θ を1回動かすたびに走る。R が最も苦手な
形(スカラ演算の深いループ)だったので、C++ 化の効きしろが大きかった。

加えて**アルゴリズム的な改善**を1つ入れた。θ(b,r) を動かしても影響を受けるのはランク r と
r+1 の2行だけ(第2添字は隣接ランク対の番号)。R版は毎回 `iso_build_pi` で全行を作り直して
...
モデル関数レベル(J35S500 ordinal isotonic, nfld=5)。EM 反復数も適合度も完全に同じ。
...
**コア単体100-500x に対しモデルレベルは25x**。差の分は E-step など R のまま残っている
部分で、**次のボトルネックはそこ**。さらに速くするなら E-step を狙う。

### 構成

- `src/isotonic_core.cpp` 新設。`iso_build_pi_cpp` / `iso_upper_cum_cpp` /
  `iso_dual_map_cpp(Mcount, maxiter, tol, fast)` をエクスポート。`fast=FALSE` は
  忠実移植(全行再構築)で、検証用に残してある。
- `R/00_isotonic_CORE.R`: 従来のR実装を **`iso_dual_map_ref()`** にリネームして参照実装
  として残置(@noRd・非公開)。新しい `iso_dual_map()` は C++ を呼ぶだけのラッパで、署名も
  返り値も不変。呼び出し側(`12_LRA_ordinal.R:350`, `16_Biclustering_ordinal.R:270`)は無変更。
- テスト2本追加。(1)5つの行列で `iso_dual_map()` と `iso_dual_map_ref()` を
  **`expect_identical`** で比較。(2)R参照が遅すぎて回せない大きさ(20ランク×6カテゴリ)で
  行和1・正値性・ランク方向の単調性を直接確認(GEM打ち切りの残差スラック 1e-3 を許容)。
- roxygen再生成で `man/Biclustering_IRM.Rd` も更新(micの記述修正ぶん)。DESCRIPTION の
  roxygen2 8.0.0 churn は commit 前に `git checkout` で戻した(既定の手順どおり)。

**確認**: 全テスト **5232 pass / 0 fail**(既存スナップショットは1つも動かず＝数値一致の
裏付け)。`R CMD check --as-cran` は **0 error / 0 warning / 2 NOTE**
(Days since last update: 6 と ローカルの HTML Tidy が古い。どちらも無害)。

### これが何を変えるか

順序 isotonic はパッケージ最遅で、**大規模シミュレーションの足枷**だった。旧Phase 3の
実測は小条件(100名×15項目)でグリッドサーチ1回あたり B 3.6秒 / R 3.1秒 / IRM 0.5秒。
isotonic を素のRで回すと桁で膨らんで非現実的だったが、C++化で同じオーダーに戻る目処が
立った。243条件×200試行のやり直しが視野に入る。**ただし要実測**(memory:
feedback_simulation_calibration = 小データからの線形外挿は外れる)。

**次**: (a)代表条件で per-trial 時間を実測してシミュレーション規模を見積もる、
(b)MSPSグリッドサーチ(ncls 2-15 × nfld 2-10)はC++版で再実行中・結果待ち。

## 2026-07-21 プロファイリングで当て推量を2つ潰す＋LDLRAのベクトル化 (commit ca9fe6c)

ユーザ「E stepのC++化、そんなのやれとは言ってないけど、効果ありそう？」で測ったところ
**私の見立てが外れていた**。以後、最適化は必ず測ってから。

### 訂正1: E-stepはボトルネックではない

C++移植後の `Biclustering(J35S500, ncls=6, nfld=5, method="R", estimation="isotonic")`
...
→ Future Work の「E-step C++化」を削除し、「速くしたいならソルバ自体か呼び出し回数。
E-stepではない」と実測つきで書き換え。

**ソルバ内部の内訳(理屈)**: 二重の二分探索がどちらも `hi-lo > 1e-10` を初期幅1から
やるので **各34回**、外側1反復ごとに内側が丸ごと走るので θ を1個決めるのに build_row が
**約2,300回**。λ にニュートン法を入れれば34→5で約6倍、外側にも同じ工夫で10-20倍は
...
→ Future Work の「GA/PBIL を C++ 化」は的外れ。**呼ばれている側**を狙う。

### LDLRAのベクトル化(実施)

`09_LDLRA.R` の `pat01` を埋める三重ループ(S×J×C=22,500回)。中身は親項目の回答を
...
**罠**: 欠測は `U` に `-1` で残る。元コードは `which(...==1)` なので -1 は 0 扱い。
素直に `U %*% w` にすると **-1 が重みごと引かれて静かに壊れる**。指標 `(U==1)` に
してから積を取ること。コミットメッセージにも明記した。

**結果**: `LDLRA_PBIL(J15S500, ncls=3, population=20, maxGeneration=20)` **15.3秒→3.8秒(4x)**。
当該ループ単体は **42倍**。検証=旧新を切り出して欠測混じり・親数まちまちで `identical=TRUE`、
`test-ldlra.R` 14件(Mathematica参照)そのまま pass、全テスト5232 pass。
**再プロファイルすると突出箇所は消えた**(最大18%、あとは分散)。ここで止めるのが妥当。

### igraph脱却をv2.0.0項目として登録

棚卸し結果: 本当にアルゴリズムなのは `is_dag`×2 と `is_connected`×2 の**4呼び出しだけ**
(08A_BNM.R:170-171, 11_BINET.R:282-283)。残り23は形式変換の往復
(`graph_from_adjacency_matrix`×10 / `graph_from_data_frame`×7 /
`as_adjacency_matrix`×3 / `as_data_frame`×3)。**v2.0.0送りにする理由は2つ**:
LDLRA/LDB/BINET が `g_list` に igraph オブジェクトを返していること、
`R/00_print_network.R` が `plot.igraph()` + `layout_on_grid()` で描画していること。
...
- **F=4は出るが条件つき**。クラス数3・4・5のときだけ nfld=4、6で3、**7以降はずっと2**。
...
**A3への含意**: 一撃にこだわらない判断(ユーザ 2026-07-21「方法論論文はシミュレーションが
...
**むしろ面白いのは IRM と BIC が逆方向を向いていること**。IRMは高解像度選好で
bfi F=14 / SAT12 F=28。BICはフィールドを最小の2へ潰す。同じデータで構造推定の基準が
正反対に振れる。§3のCRP濃度とフィールド過剰選択の議論と対になる。考察としてはこちらが強い。

**計算コストの数字**: 126セルで2.43時間＝1セル平均70秒(C++化後、MSPS 821×20)。
旧シミュのグリッド範囲は 2≤C≤20, 2≤F≤20 の **361セル**で、これを48,600試行ぶんは
このデータサイズでは不可能。ただしシミュのデータはずっと小さい(例 100×15)ので単純換算は
できない。**設計を決めてから代表条件で実測**すること
([[feedback_simulation_calibration]] 小データからの線形外挿は外れる)。

**次**: (a)A3の主張と腕(データ型×手法×基準)の設計を決める、(b)決めたら代表条件で
...
- **fitness評価のBIC専用カーネル化**: `BNM_GA`/`BNM_PBIL`/`LDLRA_PBIL` は候補ごとに
  フルの `BNM()`/`LD_param_est()` を呼んでいた（CCRR_table・sprintf・igraphオブジェクト・
  DAG検査・TestFitのベンチマークモデル再計算まで毎回）。GA/PBILの候補は上三角行列
  ゆえ常に単純DAGなので検査自体が不要。新設:
  - `BNM_bench_stats()` — TestFitのベンチマーク統計（ell_B, bench_nparm）はデータのみ
    依存なのでループ外で1回。
  - `BNM_fit_BIC()` — adjをラベルソート（fill_adjと同じ）→ カウント → loglik/nparam →
    BICを calcFitIndices と項ごとに同式で直算。
  - `LD_param_est(fit_only = TRUE, bench = )` — test_ell/model_nparam 算出後に即return。
    postdist/irp の4次元配列ブロック（S×J×C×npapat数本）を丸ごとスキップ。
- **BNMのPIRP集計ベクトル化** (`BNM_pirp_counts()`): 被験者×項目の二重ループ
  （rev/which、プロファイルで約60%）と S×J×2^npa one-hot配列を、項目ごとの
  行列積＋`rowsum()` に置換。LDLRAで確立した手筋の移植。罠も同じ=欠測は-1なので
  指標は `(U == 1) %*% w` 必須。なお歴史的セマンティクス
  `colSums(tmp$U * PIRP_array)` は欠測セルの-1がn_PIRP_1に混入する仕様で、
  これも忠実に再現した（rowsumで生のUを合計）。挙動変更はしない。
- **fill_adj()**: 要素単位の二重ループ→一発のインデックス代入。
- **BNM_GA(crossover=2)**: `combn(gene_length, 2)`＋フィルタを子個体ごと→ラン1回に
  巻き上げ（J=35でgene_length=595、約17万列の行列を毎回作っていた）。RNG消費順は不変。

**計測**: `BNM_PBIL(J35S515, pop=20, gen=15)` **14.8s→0.68s(22x)**、
`LDLRA_PBIL(J15S500, ncls=3, pop=20, gen=20)` 3.8s→**1.08s**（7/21のPIRPベクトル化と
累積で元の15.3sから**14x**）。

**検証**: 編集前HEADで基準値RDSを採取→編集後に同一seed・同一引数の9ケース
（BNM×2データ、BNM_GA交叉0/1/2、BNM_PBIL estimate1/4、LDLRA、LDLRA_PBIL）を
全フィールド `identical()` 一致。カウントが整数でBIC式も項ごとに同一なのでFPも厳密一致。
全テスト5232 pass、roxygen警告0、R CMD check（vignetteスキップ起因の警告のみ）。

**再プロファイル**: 改良後は突出箇所なし（最大は rowsum 内部の unique/sort で16%）。
**C++化は不要と判断**。パッケージ全体の次のボトルネックは既知のとおり emclus E-step。

**LDB/BINETも精査したが対象外**: どちらも探索ループ内で呼ばれない一発もの。
LDBの被験者×クラス×フィールド三重ループ(10_LDB.R:251-259)と4次元aperm/apply連鎖は
同じ手筋で直せるが優先度低。BINETは小さいループのみ。

**引き継ぎ**: 変更はcommit aeb1f2bでpush済み。NEWS.md Performanceに3項目記載済み。
...
**§4.7 冒頭のラグランジアン，第1項の係数が $\theta$ になっていた**。正しくは $\gamma$。
第1項は対数尤度なので係数は観測度数側の量($\gamma_{jcq}=U^{(t)}_{jcq}+\alpha_{jcq}-1$)であり，
乗数 $\theta$ ではない。同じ節の末尾で展開しきった式は既に $\gamma$ になっていたので，
冒頭だけの書き間違いと確定。落ちていた $c$ の和と範囲も補った。

### 機械的な修正

- **Σの上下を全箇所に明示**。`\sum_c` `\sum_q` `\sum_{q\ge k}` `\sum_s` 等16パターンを潰した。
  上下なしのΣは残っていない。
- **ラグランジュ乗数の説明を削除**。「制約を破った分だけ罰則が課される…罰則の大きさである」の
  2文を落とし「$\lambda_{jc}$ がラグランジュ乗数である」のみに。先生の指摘=「罰則の大きさ」は
...
- **KKT脚注**: Karush--Kuhn--Tucker。1951年Kuhn&Tuckerで広まり，Karushの1939年修士論文が
  後に判明して三人併記に。等式制約だけならラグランジュで足りるが不等式が入るので正確にはKKT。
  4条件のうち**相補スラック性が $\theta_{jcq}$ の更新手続きそのもの**。
  初出が§4.4.1なので，先に§4.5へ置いた脚注をこちらへ移動。
- **Fenchel脚注**: 出典の三層(原典Fenchel 1949 / 整備 rockafellar1970 / 多項尤度への応用
...
- 引用に `elbarmi1994`・`rockafellar1970` を追加使用。Fenchel(1949)単体はbib未登録のため脚注で名前のみ。

### 疑似コード追加と並べ替え(先生の提案)

- **§4.9のサブサブセクションを $\theta\to\lambda$ の順に並べ替え**(旧: $\lambda$が先)。
  冒頭が「$\theta \to d \to \lambda \to \pi$ の順に更新する」と宣言しているのに順序が逆だったため。
  前方参照が生じないことは確認済み($\theta$節はC=2,Q=3の例を閉形式で解いており
  $\lambda$の二分探索に依存しない)。
- **疑似コード(algorithm/algpseudocode)を§4.9冒頭に追加**。`Eval(θ)` 手続きを立てて
  $\theta\to d\to\lambda\to\pi$ を一本道で流し，外側ループが $\theta$ を探索する形。
  **二分探索が二重**($\lambda$が内側・$\theta$が外側)である構造が目で見えるようにした。
  「$\theta_{jcq}$ が影響するのはランク $c$ と $c+1$ の2行だけなので `Eval` の再構築は2行に限ってよい」
...
- **LRA.nominal は原理的に成立しないので作らない**(確定)。LRAのランクの順序は
  順序づいたカテゴリから意味をもらっている(上位ランクほど確率的に高いカテゴリを選ぶ)が，
  名義にはその錨が無い。GTMフィルタで1次元に並べること自体は機械的に可能だが，
  並びの向きに解釈が与えられないので「順序のあるランク」ではなく
  「直線状の位相を課しただけの潜在クラス」にすぎない。同じ理由で isotonic も名義には
  入れられない(制約が累積確率 P(>=q) 上の確率的順序で定義され，">=q" はカテゴリが
  順序づいて初めて意味を持つ)。
- 結論として **サポート行列の「—」は未実装ではなく該当なし**と読む。CLAUDE.md の表記も
  そう改める(未着手)。作るのは LCA.nominal ひとつ。
- **LRA に名義が渡されたら黙って落とさず引き渡す**: `LRA.default` の分岐に
  `response.type == "nominal"` を追加し(現状は `response_type_error` で停止)，
  「ランクは順序づいたカテゴリを要するので名義には定義されない，LCA.nominal として
  分析する」と告げて委譲する。
- 実装着手: `R/00_EMclus_nominal.R` (未エクスポートのEMエンジン)。積多項分布の有限混合
  (クラスごとに項目ごとの独立なカテゴリ分布)。クラスにもカテゴリにも順序を課さない点が
  `emclus()`(Filフィルタで隣接ランクを平滑化)や `00_isotonic_CORE.R` との違い。
  ラギッドなカテゴリ数に対応(項目ごとに ncat が違ってよい，余ったスロットは終始ゼロ)。
- **未着手**: `LCA.nominal` 本体(05_LCA.R)，`LRA.default` の分岐，NEWS.md。
  エンジン単体ではユーザ可視の変更がゼロなので NEWS は LCA.nominal 完成時にまとめて書く。

### M2 メモ (develop/Algorithm_M2.tex，18ページ相当・ビルド済み)

読んだ論文は develop/ に日本語版を作ってある(MaydeuJoe2003_ja / MaydeuJoe2005_ja /
Collins1993_ja / Reiser1996_ja)。メモの骨子:

- **飽和モデルを必要としないのが要点**。名義の応答パタン表はセル数 $\prod_j Q_j$
  ($J=15$, $Q=5$ で $3\times10^{10}$)で，標本数百〜数千ではほぼ全セルが0か1になり，
  飽和モデルの対数尤度が0に張り付いて $\chi^2_A = 2(\ell_B-\ell_A)$ が $-2\ell_A$ に退化，
  CFIが1に貼り付く。$M_2$ は比べる相手を「1次・2次マージンだけを完全に再現するモデル」に
  取り替える。土俵がパタン表からマージン表へ移るだけで発想は同じだが，$J=15,Q=5$ でも
  マージンは $m=1{,}740$ 本しかなく，1本あたりに人数が乗るので潰れない。
  $\ell_B$ がどこにも出てこない。
- **マージン本数** $m=\sum_j(Q_j-1)+\sum_{j<j'}(Q_j-1)(Q_{j'}-1)$。二値なら $J(J+1)/2$ で
  Christoffersson(1975)の自由度に一致。パラメタ数は LCA で $t=(C-1)+C\sum_j(Q_j-1)$，
  バイクラなら項目がフィールド内でパラメタを共有するので $t=(C-1)+C\sum_f(Q_f-1)$ と小さくなり
  df が大きくなる。$m$ は観測側だけで決まりモデルによらない。
- **部品は5つ**: 残差 $e=p_2-\pi_2$，ヤコビアン $\Delta$(すべて閉形式・積分なし)，
  マージン共分散 $\Xi[a,b]=\pi_{a\cup b}-\pi_a\pi_b$(和集合が肝で，最大4次のマージンが要る。
  LCAなら積の和で出るので $Q^J$ 次元の同時確率も選択行列も不要)，
  重み行列 $W=\Xi^{-1}-\Xi^{-1}\Delta(\Delta^\top\Xi^{-1}\Delta)^{-1}\Delta^\top\Xi^{-1}$，
  そして $M_2=S\,e^\top W e \to \chi^2_{m-t}$。
  **$W$ はフィッシャー情報行列を要しない**(情報行列は全セル走査が要るので実装上の大きな利点)。
  推定量が $\Xi^{-1}$ 重みの二次形式を最小化していれば第2項が消えるが，我々はEM最尤なので
  この退化は起きず $W$ が必須。
- **名義で使える根拠**: $M_r$ はカテゴリのラベル付け替えに不変(Maydeu-Olivares & Joe 2006)。
  併合は不要。mirtの `M2*`/`C2` は次元膨張を避けるための順序専用の廉価版で名義には使えない。
  名義では素の $M_2$ の計算量を払う。
- **接続先**: `calcFitIndices(chi_A, chi_B, df_A, df_B, nobs)` はカイ二乗の算出経路を問わないので，
  $\chi^2_A=M_2$・$\mathrm{df}_A=m-t$，ベースラインを独立モデルとして
  $\mathrm{df}_B=\sum_{j<j'}(Q_j-1)(Q_{j'}-1)$ (=2次マージンのセル数。独立モデルは1次を
  完全再現しずれが全部2次に出る)を流し込めば NFI/RFI/IFI/TLI/CFI/RMSEA/AIC/CAIC/BIC が出る。
  したがってこれらは「項目間の連関のうちモデルが説明した割合」を測る。
  `bench_log_like` だけは対応物が無く NA。**$M_2$ のRMSEAを全情報版と直接比較してはならない**
  (自由度が桁違い)。
- **手で追える数値例**: 独立モデル・$J=3$・$Q=2$・$S=100$ で $M_2=5.52778$, df=3, $p=.13699$。
  $\Xi$ の固有値・$\mathrm{rank}(W)=m-t$・$W\Xi W=W$・$e=0$ で $M_2=0$ まで数値で確認済み。
- **実装チェックリスト**を1節設けた。二値2PLで mirt の `M2(fit, type = "M2")` と数値一致を
  取ってからエンジンを固め，参照実装の無い名義へ持っていく。
  mirtを数値の参照に走らせるのは問題ないが **mirtはGPL・exametrikaはMITなのでコードの
  取り込みは不可**。実装は論文から独立に起こす。
- **系譜**: Christoffersson(1975)で骨格が完成 → Muthen(1978)はWLSMVの出発点だがLCAには
  対応する再パラメタ化が無いので倣うのは前者 → Reiser(1996)は一般化逆を使うため多重共線に弱く
  式(W)の直交補元形がその改良 → Collins(1993)は2クラスLCAで $G^2$/$X^2$ がカイ二乗から
  外れることを実証($X^2$ は期待値は近いが標準偏差が3倍)し処方箋がモンテカルロ。
  $M_2$ はその置き換えにあたる。
- **規模**: $\Xi$ は $m\times m$ 密行列で逆行列が律速。MSPS(J=20,Q=6, m=4,850)で約188MB，
  bfi(J=25,Q=6, m=7,625)で約465MB，J35S500(m=9,660)で約747MB。
  Maydeu-Olivares & Joe(2006)の「$J>15$ でも計算不能になりうる」は2005年当時の判断で，
  現在の計算機なら通るが無料ではない。
...
前日の続き。LCA.nominal の適合度として使う $M_2$ のメモを，ユーザと一文ずつ
読み合わせながら推敲した。develop/ はビルド対象外なので commit なし(PDFのみ更新)。
現在14ページ・lualatex+biber・エラー0。次回はこの読み合わせの続きから。

### この日入れた変更(すべて反映・ビルド確認済み)

- **冗長な一文を3つ削除**: 「そしてこの測り方には，飽和モデルの対数尤度 $\ell_B$ が
  どこにも出てこない」/「つまり2次マージンは，測るべきところを測っている」/
  「$\bm{\Xi}^{-1}$ の役はこれである」。いずれも直前で言い切っている内容の反復。
- **「マージンの作り方」冒頭**を，2種類しか使わない宣言＋観測側/モデル側の2通りを
  予告する導入に書き換え。
- **`colMeans` などR関数を数式に置換**。観測側の1次・2次マージンを $\Sigma$ で明示
  ($p_j=\tfrac{1}{S}\sum_s u_{sj}$ 等)。多値はワンホット $v_{s,jq}=\bm{1}[y_{sj}=q]$ を
  定義して同形に。$\bm{U}^\top\bm{U}$・$\bm{V}^\top\bm{V}$ の対角/非対角を `cases` で明示。
- **見出し「観測側。」「モデル側。」→ subsection「観測から求める」「モデルから求める」**。
  「〜側」は書き手の内輪の呼び名で読者に何の側か伝わらない，との指摘。
- **部品4(重み行列 $\bm{W}$)を大幅加筆**。ユーザが気に入った「出発点は残差の漸近分布」の
  説明を本文化。構成: (1) $\sqrt{S}\bm{e}\to N(0,\bm{\Xi})$ でマハラノビス距離が要る理由，
  (2) デルタ法(モデル側マージンの1次展開)→係数行列が部品2の $\bm{\Delta}$ だと接続。
  デルタ法の定義は脚注に。(3) 残差生成行列 $\bm{M}=\bm{I}-\bm{\Delta}(\cdots)^{-1}\bm{\Delta}^\top\bm{\Xi}^{-1}$
  を定義し $\bm{W}=\bm{\Xi}^{-1}\bm{M}$，GLSの残差生成行列そのものだと明示。OLS/GLS/$M_2$
  の対応表を挿入。「$M_2$ はマージン残差 $\bm{e}$ を $\bm{\Delta}$ に $\bm{\Xi}^{-1}$ 重みで
  回帰した残差平方和」を独立文で言い切り。(4) 新subsection「座標を取り替えて見る」で
  $\tilde{\bm{e}}=\bm{\Xi}^{-1/2}\bm{e}$ を導入し直交射影 $\bm{P}$ を定義，
  $M_2=S\|(\bm{I}-\bm{P})\tilde{\bm{e}}\|^2$ から極限 $\chi^2_{m-t}$ を出す。検証節の
  $\bm{W}\bm{\Xi}\bm{W}=\bm{W}$・$\mathrm{rank}=m-t$・$\bm{W}\bm{\Delta}=0$ がすべて系だと明示
  (検証節に `\label{sec:check}` を付与)。**「白色化」の語は使わない**方針(ユーザ指示)。
- **カテゴリ番号0始まりの誤りを修正**(コードで確認)。`dataFormat()` はファクタを
  $1,2,\dots$ に変換し(01_dataFormat.R docstring L29)，名義は任意ラベル扱いで非負整数
  チェックもスキップ(L304-305)。EMエンジン `00_EMclus_nominal.R` は `as.vector(Q)[valid]` を
  三次元配列の添字に使うので `1:ncat[j]` 前提。よってメモの「各項目のカテゴリ0を落とす」は
  誤り。表記節を「実装上は $1,\dots,Q_j$ の整数。記法上どれか1つを基準に選んで0と呼ぶ。
  $M_2$ はラベル付け替えに不変(\S\ref{sec:nominal})なので基準の取り方によらない」に修正。
  V行列の説明も「$\sum_q v_{s,jq}=1$ で1本は残りから決まるので基準カテゴリの1本を落とす」に
  (0の明示は表記節へ一本化)。名義節に `\label{sec:nominal}` を付与。
- **対角ブロック退化の説明を中身のある文に**。「退化する(理由列挙)」という結論だけの
  書き方をやめ，「対角ブロック($j=j'$)には2次マージンが入らない。$q\ne q'$ は
  $v_{s,jq}v_{s,jq'}=0$ で消え，$q=q'$ は $v_{s,jq}^2=v_{s,jq}$ で1次マージン $p_j(q)$ に
...
まだ一文ずつのレビュー未通過)。その後 LCA.nominal 本体と $M_2$ の実装へ。

## 2026-07-25 M2メモの読み合わせ完了

前日の続きで develop/Algorithm_M2.tex を最後まで読み合わせた。develop/ はビルド対象外
なので commit なし(PDFのみ更新)。14ページ・lualatex+biber・エラー0を維持。
これでメモは全節がレビューを通過した。

### 数値例の再検証

メモの $J=3$・$Q=2$ 独立モデルの例をRで再実行し，記載値と完全一致を確認した。
$M_2=5.52778$，$p=0.13699$，$\mathrm{rank}(W)=3$，$W\Xi W=W$ の残差 5.3e-15，
$\Xi$ の固有値 $(.5208,.3549,.2886,.0499,.0406,.0277)$。あわせて $\Xi$ の全16成分と
$\Delta$ の各行を手で検算し，誤りがないことを確認した。規模の表($m$ とメモリ量)も
再計算して一致。

### この日入れた変更(すべて反映・ビルド確認済み)

- **$M_r$ の初出を修正**。名義節冒頭を「$M_2$ を含む $M_r$ 族は」に。$M_r$ の定義が
  出るのは系譜節なので，それより前で無定義に使っていた。
- **ベースライン側の記号を分離**。適合度接続の $\chi^2_B$ を $M_2^{\mathrm{null}}$ とし，
  「独立モデルに対してまったく同じ手順で計算し直した $M_2$」と明示。$\chi^2_A$・$\chi^2_B$
  が同じ $M_2$ と書かれていて，2回計算する手順が読み取れなかった。
- **RMSEA注意の理由を明示**。「自由度が桁違いだから比較するな」だけだったものを，
  RMSEA が $\sqrt{(\chi^2-df)/(df\cdot S)}$ と自由度で割る量であることから書き下す3文に。
- **「2005年当時」→ \textcite{maydeuolivares2005} 当時**(直前の引用は2006年版で不整合)。
- **「その標本版を $p_2$ として」を廃止**。「モデルから求めたマージンを縦に積んだ
  ベクトルを $\pi_2$，同じ並びで観測から求めたものを $p_2$ として」に。直前の節の
  小見出し(「観測から求める」「モデルから求める」)と呼び方を揃えた。
- **式番号の粒度を修正**。ヤコビアンの4式が2列組みで2番号しか付いていなかったのを
  1式1行に分割。3変量・4変量マージンの2式も同様に分割(align化)。
- **$\Delta$ の張る平面の説明段落を新規追加**(式\eqref{eq:delta}の直後)。「残差は $m$ 次元に
  自由に散らばるのではなく $\Delta$ の列方向のずれはモデルが消している」が伝わらなかった
  ため，$\Delta$ の列＝パラメタを動かしたときのマージンの移動方向，$t$ 次元の平面上で
  $p_2$ に最も近い点を選ぶのが推定，選び終えた時点で平面方向に残差が残っていたら
  最善という前提と矛盾する，という順で書き下し，1階条件
  $\Delta^\top\Xi^{-1}e=0$ で締めた。回帰の残差が説明変数と直交するのと同じ話。
- **見出しの付け替え3つ**。
  - 「座標を取り替えて見る。」→「斜交射影を直交射影に直す。」(何をどう取り替えるのかが
    題から分からなかった。段落の操作そのものを題に)
  - 「なぜ $W$ が要るのか。」→「$\Xi^{-1}$ だけで済む推定法もある。」(中身は $W$ の必然性
    ではなく，退化する例外条件と我々がそこに当てはまらないこと。$W$ の導入は L229 からの
    冒頭で済んでいる。なお第2項が消える議論は1階条件を出した後でないと書けないので，
    この段落を冒頭へ移すことはできない)
  - フィッシャー情報行列の2文を独立段落にし「情報行列は要らない。」を付与(退化の話とは
    別件の実装上の利点が同居していた)。部品4は 本体→例外条件→実装上の利点 の3段に。
- **「系として出る」を平易化**。「第8節の検証項目は，独立した3つの性質ではなく，すべて
  式\eqref{eq:WP}から出る。$I-P$ が直交射影であること一つが効いている」に。
...
M2メモは完了。実装フェーズへ。順序は (1) LCA.nominal 本体(仕様確定済み・EMエンジンのみ存在)，
(2) $M_2$ の実装(二値2PLで mirt の `M2(fit, type = "M2")` と数値一致を取ってエンジンを
固めてから名義へ。mirtはGPLなのでコード取り込み不可・参照実行のみ)。
CLAUDE.md のサポート行列表記を「該当なし」に改める件も残っている。

追記(同日): 規模節の「現在の計算機なら通るが無料ではない。」を「現在の計算機なら通るが，
数百MBのメモリと相応の計算時間を要する。」に修正(英語の not free の直訳で，日本語では
金銭の話に読める)。メモは荘島先生へ送付済み。
...
7/25の続きで全節を通した。$M_r$ の初出を修正，適合度接続のベースラインを
$M_2^{\mathrm{null}}$ と書き分け，RMSEA注意の理由(自由度で割る量である)を明示，
「2005年当時」を \textcite に，「その標本版」→「同じ並びで観測から求めたもの」，
ヤコビアン4式と3・4変量マージン2式の式番号を分割，$\bm{\Delta}$ の張る平面の
説明段落を新規追加(残差が平面方向に成分を持たない理由を一階条件まで書き下し)，
見出し3つ付け替え(「座標を取り替えて見る」→「斜交射影を直交射影に直す」，
「なぜWが要るのか」→「$\Xi^{-1}$ だけで済む推定法もある」，情報行列の段落を分離)，
「系として出る」「最初の関門」「無料ではない」を平易化，節見出し2つ短縮。
**荘島先生へ送付済み**。ただし後述の $\gamma_c=1/C$ 固定に合わせた式の修正は未反映。

### 2. LCA の S3 化と LCA.nominal / LCA.rated (R/05_LCA.R)

- `LCA()` を総称関数化。`LCA.default`/`.binary`/`.nominal`/`.rated`。
  binary の結果は不変(log_lik -3663.994, 73サイクルで従来一致)。
- **LCA.nominal**: `emclus_nominal()` を呼ぶだけで新EMコードなし。ICRP(item×category行 ×
  class列)・LCD・CMD・Students を返す。母数は `ncls * sum(ncat - 1)`
  (混合比は荘島流で $1/C$ 固定なので数えない)。適合度は AIC/BIC/CAIC のみ
  (飽和モデルを作らない理由は Biclustering.nominal と同じ)。
- **LCA.rated**: 名義で推定 → 正答による後処理。IRP はモデル含意の正答カテゴリ確率
  $\rho_{j,CA_j|c}$(Biclustering.rated の経験的正答率とは異なる)，TRP はその重み付き
  項目和。適合度2層(二値層=カイ二乗系あり，名義層=情報量規準のみ)。
  **クラスの正答率ソートはしない**(LCAのクラスは無順序)。
- **ordinal は nominal へ委譲**(メッセージ付き)。クラスが無順序なのでカテゴリの順序を
  つなぐ相手がなく，推定は名義と数学的に同一。逆に **LRA に名義が来たら LCA へ委譲**
  (`nrank` は `ncls` に読み替え)。サポート行列の "—" を "n/a"/"via nominal" に書き分け。
- print・plot 対応。ICRPプロットは**線でなく群棒グラフ**(クラス軸もカテゴリ軸も無順序で，
  線は存在しない傾向を示唆するため)。テスト23本追加。

### 3. M2 の試作 (develop/20260725_M2_prototype.R)

- メモの数値例($J=3,Q=2,S=100$の独立モデル)を $M_2=5.52778$, df=3, $p=0.13699$ まで
  完全再現。$\bm{\Xi}$ の全成分・$\bm{\Delta}$ の全行も一致。
- $\bm{\Xi}$ をモンテカルロで検証(2000反復で最大差0.011，最大値0.25)。
- **実装上の要点**: $\bm{\Xi}$ の非対角は $A A^\top/C$ 一発。項目が重なる場合だけ補正
  (同一項目の別カテゴリは0，同カテゴリは重複因子を割る)。同じ項目対のペアマージン同士は
  ブロック対角(値は $\pi_a$)。この2段補正を忘れると対角が壊れる。
- **発見: $\bm{\Delta}$ が $(C-1)(C-2)/2$ だけランク落ちする**。クラス比率固定だと
  2次マージンはクラス偏差行列のグラム行列にしか依存せず，$(C-1)$ 次元空間の回転で不変。
  $C\ge3$ でLCAの母数は2次マージンから識別されない($\pi_2$ 自体は識別される)。
  よって自由度は $m-\mathrm{rank}(\bm{\Delta})$。SVDで射影を作れば数値的にも安定。
- J20S600($m=1770$)で1秒未満。

### 4. EM収束判定の欠陥(本セッション最大の発見)

M2の帰無分布検定で平均 $M_2$ が80.7(期待値37)と大きく外れたことから発覚。原因は
推定量が最尤解に達していないこと。3つの欠陥がありいずれも**荘島実装由来**で，Rは
忠実に移植していた(`develop/mtmk15forVer13/mod/Module_IRP.wl` で確認)。

1. **初期値**: `-1/const = -exp(J)` は「どんな対数尤度より下」のつもりだが，短いテストでは
   上に来る($J=6$ で $-403$ 対 実際 $-3900$)。1周目終わりに「減少」と誤判定し，
   `converge = TRUE` のまま1周で終了。二値LCA(6項目1000名)・`J5S1000` の
   Biclustering.ordinal で実測。$J\ge15$ のデータしか参照テストになかったので未発覚。
2. **監視量**: 本の式(5.11)(5.12)は $\ln pr(\bm{\Pi}^{(t)}|\bm{U})$ の単調増加を主張するが，
   与えられている計算式は期待対数事後 $Q(\theta_t\mid\theta_{t-1})$。EMが保証するのは前者で
   あって後者ではない。6項目データで iter1 $-3869.26$ → iter2 $-3922.42$ と減少し，
   「減ったら巻き戻して停止」が正常な反復で発火する。
3. **許容誤差**: 相対 $10^{-4}$ は値が数千なら実質0.4ネイト。`J15S500` 5クラスで
   17〜73周で停止(観測対数尤度 $-4121.53$)，厳密収束は440周で $-4118.16$。

**修正**: 監視量を観測データの周辺対数尤度に変更＋初期値 $-\infty$＋許容誤差 $10^{-8}$＋
`maxiter` 既定 100→1000 を `emclus()`(二値LCA・LRA GTM・LDLRA)，`emclus_isotonic()`，
`emclus_nominal()` に適用。バイクラスタリング(07/15/16)は**保守的に番兵と許容誤差のみ**
(二重混合で観測データ尤度が一意に決まらないため。ユーザ判断)。IRT(04C)は許容誤差
$10^{-5}\to10^{-6}$(1周ごとに項目数ぶんの optim を回すので $10^{-8}$ は4.86秒と高価，
かつ optim の reltol に律速される)。

**Mathematica側も同一の修正**を入れ，`Module_IRP.wl`(完全)と `Module_Biclus.wl`・
`Module_IRT.wl`(保守的)を更新。R と倍精度まで一致(LCA J15S500 5クラス: 337サイクル，
$-3651.903821773122$ 対 $-3651.9038217731227$)。修正後にM2の帰無分布が理論値と一致
(平均35.32 対 自由度37，棄却率0.030)。

**罠**: Mathematica でも `emt > 0` ガードでは不十分。2周目に `oldtestell` がまだ
`-Infinity` で `Infinity <= Infinity` が真になり停止する。`NumberQ[oldtestell]` が必要。
R 側も同じ理由で `is.finite(old_test_log_lik)` を使う。

### 5. IRT の高速化

プロファイルで M-step の目的関数が自己時間の80%。(a)応答関数を $p$ と $1-p$ で2回
呼んでいたのを1回に，(b)スカラーへの `ifelse` を `if` に，(c)**解析的勾配を optim に供給**
(勾配なしL-BFGS-Bは有限差分で母数の数だけ余分に目的関数を呼ぶ)。
`IRT(J35S515, model=2)`: 1.55秒→0.34秒(締める前の0.63秒より速い)。母数差は5e-6。
勾配は数値微分と2PL/3PL/4PLすべてで $1.5\times10^{-7}$ 以内で一致。

### 6. フィクスチャ再生成

`develop/mtmk15forVer13/tools/xlsx2fixture.R`(readxl + write.csv。**旧xlsxから既存
フィクスチャをバイト単位で完全再現できることを検証済み**)と `tools/regen_fixtures.sh` を整備。

- **差し替えた**: Chapter04IRT{2,3,4}pl(12) / Chapter05LCA(4) / Chapter06LRA_GTMmic{0,1}(8) /
  Chapter11BINET(6) = 30ファイル
- **不要**: Chapter07 Biclustering・Ranklustering(再生成しても完全一致。減少検知で
  収束が決まり許容誤差の変更が効かない) / Chapter09 LDLRA・Chapter10 LDB(旧参照値のまま
  テスト通過)
- **注意**: Ch09 LDLRA は `J12S5000` で修正前後とも30分以上かかる。重さは実装由来で
  今回の変更とは無関係(修正前コードで実測)。またログが6.4GBに膨れてジョブが落ちたので
  `regen_fixtures.sh` は `tail -c 200000` でログを絞る。

### 方針決定(ユーザ判断)

- `.nb` は受領時のまま保存記録とし，**`.wl` を今後の正本**とする(エンジンから見れば同じ)。
  来歴と修正内容は `mod/README.md` に記載。無修正の .nb と旧xlsxは `backup_20260725/`。
- 本の数値と一致しなくなることは NEWS に明記。v1.16.0 に入れる。荘島先生には先生から説明。
- 科学的正しさを優先する，というのがこの一連の判断の基準。

### 次

1. M2メモを $\gamma_c=1/C$ 固定に合わせて修正(モデル節・ヤコビアン2式化・$t=C\sum(Q_j-1)$・
   3/4変量マージン・将来 $\gamma$ を推定する版への注記)→ 差し替え版を荘島先生へ
2. M2 の本実装(R/へ)。二値2PLで mirt の `M2(fit, type="M2")` と突合してから名義へ。
   mirt は GPL なので参照実行のみ・コード取り込み不可
3. v1.16.0 リリース準備(8/15目処)

追記(2026-07-26): M2メモを実装に合わせて改訂(16p)。$\gamma_c=1/C$ 固定への統一5点
(モデル節の式・ヤコビアン4式→2式・$t=C\sum_j(Q_j-1)$・3/4変量マージン・バイクラの母数数)，
自由度を全体で $m-\mathrm{rank}(\bm{\Delta})$ に統一，第7節「自由度は母数の数ではなく
$\bm{\Delta}$ の階数で決まる」を新設(グラム行列経由の回転不定性 $(C-1)(C-2)/2$ の導出と
J20S600での数値確認)，テキストの $\pi_c$ と本ノートの $\pi$(マージン)の衝突を表記節に注記。
荘島先生へメール送付済み(草稿 develop/20260726_荘島先生宛メール案.txt)。

**$\gamma$ を推定しても自由度は増えないことを実測で確認**: $\bm{\Delta}$ に $\gamma$ 列
($C-1$ 本)を足しても階数がまったく増えない(J20S600・C=2〜5 で 120/179/237/294 のまま)。
2次マージンからはクラス比率を動かすこととカテゴリ確率を動かすことが区別できない。
ランク落ちは $(C-1)(C-2)/2$ から $(C-1)C/2$ に増えるだけ。$\gamma$ 固定は M2 の観点でも
合理的という結論。検証コードは prototype の `check_gamma_rank()`。
...
非互換である。マイナーバージョンでは実態を過小に伝えるという理由で 2.0.0 に上げた。
どうせメジャーを上げるなら非推奨名の削除も同時に済ませる(壊すのは一度に)。

- **2.0.0 に入れる**: EM収束判定の修正 / LCA.nominal・rated / IRT高速化 / アイソトニック推定 /
  GRM監査修正 / BNM・LDLRA高速化 / **M2** / 非推奨の関数名・フィールド名の削除
  (下流の実測: ggExametrika 7箇所・shinyExametrika 11箇所。機械的置換なので同時提出)
- **入れない**(設計作業にリリースを人質に取られないため): 多値BNM(→2.1.0) / 構造学習(→2.2+) /
  igraph除去(描画経路の代替が要るのでさらに先) / 入力データ保持方式の統一
- **時期**: A3シミュレーション完了後の9月上旬。1.15.0 は 7/15 提出・数日で受理なので
...
7/21 版への差分として書いた。**7/21 の確定事項(腕3本・SOM除外・信号強度R・フィールド異質性・
132セル・試行500・GridSearch 2測定・指標・表現の統一)は動かさない**。

- **構えを書き直した**: 当初「主張A/B/C」と勝敗の形で立てたが，ユーザ指摘で**問いの一覧**に
  改めた。このシミュレーションは順序制約の優位を示す装置ではなく，多値順序モデルが条件で
  どう振る舞うかを**記述する**装置。目玉(一撃)は結果を見てから決める。
  **「一撃」の出所を取り違えていた**: SWB が「方法の話か応用の話か分からない」と指摘された
  ことへの反省で，**目的を1つに絞れという規律**であって劇的な発見の要求ではない
  (応用SWB/CLAUDE.md「攻撃→提示。淡々と道具について静かに語る」)。結論ありきで図を用意する
  ことはその規律に反する
- **私の初稿は旧設計を引きずって二値化の腕を入れていた**。7/21 に除外済み(順序を捨てることと
  解像度を落とすことが交絡する)。ログを読まずに書いた誤り
- **変更1: 適合度は尤度ベースに統一**。`log_lik` と母数の数から自分で AIC/BIC を計算し，
  返り値の `TestFitIndices$AIC` は読まない。二値・順序経路はカイ二乗ベース
  (`AIC = χ²_A - 2df_A`)，名義経路は `-2ℓ+2k` で別物(実測: 二値LCA 233.5 / 名義LCA 7471)。
  3腕は同一の多値データ上で同じ `P(Y=q)` を与えるので標本空間が共通で比較できる。
  二値化データの尤度は別の標本空間なので比較不能(腕から外した判断はこの点でも正しい)
- **変更2: クラスサイズ配分の偏りを追加**。equal / unimodal(山型) / skewed(3:1)。
  E-step がクラス事前確率を 1/C 固定している(テキスト160ページ)ので，偏れば誤特定になる。
  この代償は誰も測っていない。**空クラスは失敗扱いせず結果として記録**(ユーザ判断)。
  全条件に交差させず中心条件で +2セルのみ
- **前提**: 5月の243条件資産は使わない(枠組み変更＋EM修正で二重に無効)
- **実行設計(停電対策)**: 8/13-14 に4号館終日停電。Al-Khwarizumi は Raspi の
  `@reboot wake_al_on_boot.sh` で自動復帰するが R プロセスは死ぬ。単位を「条件×試行」に切り，
  1単位ごとに書き出し，冪等にし，一時ファイル→`file.rename()` で原子的に置換，シードは決定的。
  停電を避けて日程を組む必要はない
- **M2 は密行列版で計算し高速化しない**。$\Xi$ は「疎行列＋階数 C+1」に分解でき Woodbury で
...
**論文には書かないが計算はする**(ユーザ判断: 書かないことと計算しないことは別。探索的に
やる以上，出せる出力は出す)。A3 本文では脚注1つに留め，実装はパッケージへ。

- `M2()` を S3 総称関数として新設。`LCA.nominal` / `LCA.rated` にメソッド。適合済みモデルを
  受け取る独立関数にした(計算が重い・既存の TestFitIndices を壊さない・シミュから任意の
  フィットに呼べる)。`LCA.nominal`/`.rated` の返り値に `Q`・`Z` を追加
- 出力は M2・df・p・m・n_param・rank_delta。print は階数落ちの説明つき
- J20S600・3クラスで M2=3510.944, df=1591, m=1770, t=180, rank=179，0.2秒
- テスト4本: メモの数値例(M2/df/p に加え $\Xi$ 全36成分・$\Delta$ 全18成分)，データ=モデルなら
  M2=0，階数落ちが $(C-1)(C-2)/2$ (C=2..4)，適合済みモデルからの実行と print と誤用エラー
...
Al-khwarizumi は 80コア・188GB(空き179GB)。BLAS を1スレッドに固定する運用なので1フィットは
数倍。1プロセス10GB前後で15並列，20.1万フィットで約15日の見込み。

**C++化は効かない**: 時間の大半はコレスキー分解で，19秒は実効140GFLOPS。すでに最適化された
LAPACK が回っており，書き直しても呼ぶ先は同じ。効くのは $\Xi$ の構造(疎＋低階数)だが上記の
理由で今回は採らない。

### この日の議論で確定した理解

- **γ(クラス比率)と TRP の mic は別物**。ユーザが記憶していた「TRPの比率を補正する話」は
  7/19 の多値バイクラ `mic=TRUE`(クラスを合計期待得点で並べ替えるだけ・J35S500では恒等写像)。
  γ はクラスの大きさで，mic は並び順。掛ける対象が違う
- **PAVA/Dykstra の重みと γ は同じ量が源**。M-step のプーリング重み(`nmat = correct_cls +
  incorrect_cls` 等)は「そのランクの期待人数」で，γ を推定するならその推定値は
  $\gamma_c=\frac1S\sum_s m_{sc}$ = 同じ期待人数。違いは，M-step の重みは制約付き最尤の要件で
  選択の余地がない(Ayer 1955)のに対し，E-step の事前分布は設計判断であること
- **「1/C は事前に知り得ないから」ではない**。γ は推定できる母数であり，1/C 固定は
  「全クラスが同じ大きさ」という積極的な仮定。中立でありたいなら平坦なディリクレ事前分布で
  推定するのが筋。ただし実害があるかは別問題で，それを測るのが上記の配分因子
- **γ を推定しても自由度は増えない**(実測)。$\Delta$ に γ 列を足しても階数がまったく増えない
  (J20S600・C=2〜5 で 120/179/237/294 のまま)。2次マージンからはクラス比率を動かすことと
  カテゴリ確率を動かすことが区別できない。よって γ 固定は M2 の観点でも合理的
- **既存の alpha/beta とは別物**。`emclus_nominal` の `alpha` はカテゴリ確率 $\rho_{jq|c}$ への
  ディリクレ事前分布，`beta1/beta2` は $\pi_{jc}$ へのベータ事前分布。どちらもセルの確率への
  事前分布で，クラス比率への事前分布ではない。γ を推定可能にするなら新しい引数になる
- **M2 の p 値の位置づけ**: 帰無仮説は「モデルが母集団の1次・2次マージンを再現している」で
  あって「モデルが正しい」ではない。SEM のカイ二乗と同じ論理だが，同じ弱点(標本サイズ依存)も
  そのまま。A3 の立場(記述的・ファミリー内相対診断)からすると合否のスタンプには使わない

### 次

1. M2 をバイクラスタリング(フィールド共有の $\Delta$)と LRA へ拡張。シミュレーションで
   3腕すべてに記録するため。半日程度
2. シミュレーションのデータ生成コードとパイロット(1条件・10試行で per-trial 時間を実測)
...
- **「飽和モデルを必要としない」の表現**: 正しくは「飽和モデルの M2 は常に 0 になるので
  明示的に計算する必要がない」。飽和モデルは残差が零ベクトルなので M2=0。よって
  $M_2=\chi^2_{\text{model}}-\chi^2_{\text{saturated}}(=0)$ と読める
- **応答パタン系とマージン系を混ぜるな**(最も重要): 「僕が査読者ならそれらを混ぜてもいん？
  という突っ込みをいれてしまうかも。そのときに『いいんです！』と合理的に説明するのは
  結構難しい」。**`calcFitIndices()` に M2 を流し込む当初設計への正面からの警告**。
  代案も提示された=マージン系でも帰無モデルを定義すれば $M_2^{\text{null}}$ が計算でき，
  M2ベースのCFI/RMSEAに展開できる
...
- マージンの数え方を「本」→「個」に統一
- **新小節「応答パタン系とマージン系は混ぜない」**。ここで組む指標は $\chi^2_A$ も
  $\chi^2_B$ も M2 なのでマージン系で閉じている。混ぜてはならないのは従来の
  応答パタン系のカイ二乗と同じ式に入れること。ベースラインの選択肢(独立モデル vs
  1次マージン等分)にも言及
- **新小節「なぜ2次で止めるのか($M_3$ について)」**。(a)規模: 3次マージンは項目数の3乗。
  20項目5カテゴリで $m$ が 3,120→76,080，$\Xi$ が46GB。MSPS規模で173GB。$\Xi$ の計算には
  $r=2$ で4次・$r=3$ で6次のマージンが要る。(b)**識別可能性との接続**: 2次マージンで
  $(C-1)(C-2)/2$ の階数落ちが起きるのは偶然でなく，潜在クラスモデルの識別可能性が一般に
  3元表から得られる(Kruskalの定理を使った Allman, Matias & Rhodes 2009)ため。
  3次マージンこそが混合を識別する情報を持つ。$M_3$ ならこの階数落ちも解消する見込み。
  文献 `allman2009` を myBiber.bib に追加

### 実装1: マージン系の適合度を別建てに (commit 4c2132d)

- `add_M2(fit)` がモデルオブジェクトを返し `TestFitIndicesM2` を足す
  (brms の `add_criterion()` と同じ形。ユーザ選択)。中で当てはめモデルと独立モデルの
  M2 を計算し，**両方ともマージン系のカイ二乗として** `calcFitIndices()` に渡す
- `print()` に `fit_indices = "both"(既定)/"pattern"/"margin"`。2ブロックを別々に表示
- **マージン系にAIC/BIC/CAICは置かない**。$\chi^2-2\mathrm{df}$ が正当なのは応答パタン系の
...
- 独立モデルは1クラス・観測周辺確率。回転の不定性がないので列フルランクで，
  $\mathrm{df}_{\text{null}}$ は2次マージンのセル数そのもの(実測で一致)
- J20S600・3クラスで NFI .622 / CFI .746 / RMSEA .045。名義データで従来まったく出せなかった

### 実装2: M2 を LRA とバイクラへ (commit c824b82)

**すべて `R/24_M2.R` 内。15_/16_ には一行も足していない**(S3で返り値のクラスに振り分けるので
モデル側は無関係)。

- **LRA は新規コード不要**。順序づいたLCAなので m・母数・ヤコビアンの構造が同じ。
  推定法だけが違う。テストで m/t/rank の一致を確認
- **バイクラは `m2_delta_shared()` が要る**。1つのフィールドプロファイルを複数項目が使うので
  複数のマージンが同じ列に微分される。**加算で組む**必要がある: 同一フィールド・同一
  カテゴリのペアマージンは $(1/C)\sum_c\rho^2$ で微分に係数2が付き，代入だと黙って半分になる。
...
- **カテゴリ数が揃わないバイクラは明示的に停止**。フィールドプロファイルは常に最大カテゴリ数
  ...
  $\Xi$ が正定値でなくなり `chol()` が落ちるが，そのメッセージからは原因が読めない
- 実測: LRA(J5S1000, 3ランク) isotonic 45.00 / GTM 42.60 / LCA 43.98 (df=42, m=80, t=39, rank=38 共通)。
  バイクラ(J35S500, 4クラス3フィールド) isotonic 15362 / GTM 15070 / 無順序 15318
  (m=9660, t=48, rank=45, 2.6秒)

### メモリ回りの整理 (測ってから決めた)

- **残差ベクトルを返り値から削除**。シミュは1フィットにつき1つ保持するので，
  項目数に比例して増える量を持たせない。現在1.4KB固定
- `chol()` 直後に `rm(Xi)`。外積は列ごとに引く(`joint - outer()` は $m\times m$ を3個同時に持つ)
- **関数内の明示的 `gc()` は試して外した**。3割遅くなってメモリは減らない。ピークを決めるのは
  $\Xi$ とコレスキー因子が同時に生きる瞬間で，`gc()` はそこに効かない
- 代わりに **`gc = TRUE` 引数**(既定)。計算後に1回だけ解放。対話的利用は1モデル1回なので
  これが親切。ループからは `FALSE`。`add_M2()` は内部2回計算のうち中間を省き最後に1回
- **累積しないことを実測**: 12回連続でヒープ1.48GB一定・RSSも横ばい
- ピークをさらに下げるには LAPACK `dpotrf` によるその場分解(Rcpp)が要る。$J=50$ で
  5.8GB→2.9GB，サーバ並列度が30→60。**保留**(必要になってから)

### デモスクリプト (ユーザ確認用)

- `develop/20260727_M2_demo.R` — LCA編。8節(従来出力/M2単体/両方表示/クラス数比較/
  レーティング/計算時間/メモの数値例再現/残差ゼロならM2=0)
- `develop/20260727_M2_demo_LRA_Biclustering.R` — LRA・バイクラ編。6節(3本の腕/ランク数と
...
- 大きい問題では `verbose = TRUE` で進捗を表示する(2つの統計量のどちらを計算中か)

### 次

1. シミュレーションのデータ生成コードとパイロット
2. 非推奨名の削除と下流2パッケージ(ggExametrika 7箇所・shinyExametrika 11箇所)の更新
3. v2.0.0 リリース(9月上旬)
...
> CRAN版v1.15.0公開済。開発版v1.16.0(8/15目処)=GRM監査修正済＋LRA.ordinal混在カテゴリ対応済＋アイソトニックLRA/バイクラ実装済・新デフォルト(二値PAVA/多値Fenchel双対・memory: project_isotonic_latent_rank)＋双対ソルバC++化(ソルバ単体509x)＋BNM/LDLRA構造探索をBIC専用カーネル化(BNM_PBIL 22x・LDLRA_PBIL累積14x・全結果identical・C++不要と結論)。全テスト5232 pass。次のボトルネックはemclus E-step。Title変更(issue#32)未着手・v2.0.0多値BNM設計中。LCA.nominal/ratedは実装完了。EM収束判定の3欠陥(番兵exp(J)・監視量が期待対数事後・許容誤差1e-4)を荘島実装まで遡って修正し，Mathematica側も同時修正して倍精度一致・参照フィクスチャ30件を再生成(本の数値とは一致しなくなる＝破壊的変更)。IRTは解析的勾配で4.5倍速。全テスト5284 pass。M2実装済(R/24_M2.R・LCA/LRA/バイクラの3腕に対応・add_M2でマージン系適合度を別建て表示)。荘島先生から著作の数値変更を承認する返信＋M2への指摘3点(飽和モデルの言い方・パタン系とマージン系を混ぜるな・M3の示唆)を反映済。開発版は破壊的変更の規模からv2.0.0に改番し，非推奨名削除も同梱して9月上旬提出予定(下流2パッケージと同時)。A3シミュは7/21設計への差分を確定(尤度ベース比較・配分の偏り追加・停電対策の再開設計・M2は密版で全腕記録するが論文には書かない)。次=M2をバイクラ/LRAへ拡張→生成コードとパイロット→8月上旬に本実行。詳細→Git/exametrika/{CLAUDE,WORKLOG}.md

## 2026-08-19 — 2.0.0: 非推奨名を実際に削除し，Windows の検査時間22分を189秒へ

pkgdown の失敗を追いかけたところ，**2.0.0 の目玉である非推奨名の削除が一度も実行されて
いなかった**ことが判明した。そこから芋づる式に，削除の実施・下流の本当の検証・Windows の
検査時間という3つの問題が出た。**pkgdown を無視していたら，虚偽の申告を添えて，しかも
検査時間で弾かれる版を提出していた。**

### 発端: pkgdown が落ちる

`_pkgdown.yml` の索引に `M2` と `add_M2` が無い，という単純な失敗。ところが直そうとして
索引を開いたら，`Deprecated` 節に「2.0.0 で削除予定」として4関数が**まだ載っていた**。
実物を確認すると export も Rd も R の定義も全部生きており，旧フィールド名も R/ に111箇所
残っていた。

**`cran-comments.md` に「削除済み」と書いてしまっていた。**CLAUDE.md の「2.0.0 に含める」
という**計画**を実装済みと思い込み，実物を確認せずに CRAN 向け文書へ書いた。マージコミット
のメッセージにも同じ記述が入っていた。`R CMD check` が通っていたのは当然で，何も消して
...
1. **内部の読み出し80箇所を snake_case へ**。別名がまだ生きているので**意味は変わらない**。
   ここでテストが通ることが，読み替えが正しいことの証明になる
2. **別名の定義49行を削除**。ここで初めて破壊的変更になる
3. **非推奨4関数を削除**し，NAMESPACE と Rd を再生成

**構文を3回壊した。**`list(...)` から行を消すと空要素が残る。パターンは3つあった:
(a) 素の末尾カンマ，(b) 孤立した `# Deprecated fields` コメントの陰のカンマ，
(c) 行内コメント付きの `log_lik = testell, # New naming convention`。
毎回テストで検出でき，段階を分けた判断が正しかったことが確かめられた。

後方互換テストは**主張を反転**させた（`expect_equal(旧, 新)` → `expect_null(旧)` +
新名が存在すること）。`GridSearch()` の `"LogLik"` は残した——あれは `index` 引数の
...
ggExametrika の `R/` は `.first_non_null(data$n_class, data$Nclass, ...)` の形で新名を先に
試すので，削除は第2引数を NULL にするだけ。壊れたのは `test-DAG-plots.R` が旧名を直接
読んでいた3箇所のみで，そちらを修正して全通過（別リポジトリでコミット・push 済）。

**`ggExametrika` は exametrika を `Suggests` に置いている**（Imports ではない）。lockstep
提出は不要と確定した。

### Windows の検査時間 22分（本日最大の問題）

win-builder が `Status: OK` を返したが，**checking tests に22分**（全体1531秒中1320秒）
かかっていた。v1.13.0 は `Overall checktime 11 min > 10 min` で自動棄却されている。
**`Status: OK` は「提出しても通る」を意味しない**——win-builder はビルドと検査が通るかを
見るだけで，10分の上限は提出時の incoming 検査で適用される。

`skip_on_cran()` は効いていた（SKIP 21・"On CRAN"）。それでも22分だった。
...
**他の37本はほとんど変わらない**（test-grm.R は 26.6→26.9秒）。M2 は周辺共分散行列の
Cholesky 分解を行い，**コストは項目数の3乗**で効く。CRAN の Windows 版 R が同梱する
参照実装の BLAS は SIMD もキャッシュブロッキングも持たない素朴な三重ループなので，
**大きな行列分解でだけ桁違いに遅くなる**。`test-lca.R` の3ブロックだけが `J35S500`
（35項目）で M2 を計算していた。

**見立てを2回外した。**(1)「BLAS のスレッド数の差」→ 1スレッドに絞っても3倍にしかならず，
効いていたのは**単スレッドあたりの実装品質**。(2)「全ファイルに手を入れる規模」→ OpenBLAS
...
3つ目が鍵だった。`m2_caveat_biclustering()` を読むと **caveat は `$model` と `$estimation`
だけで決まる純粋な関数**で，データの中身も大きさも一切関係ない。**0.29秒で4つの主張すべてが
再現する。**同梱データは増やさず，テストファイル内で種を固定して生成する形にした
（`data/` に足すと配布物が太り，文書化も要り，公開APIの一部になる）。

**結果: 参照BLAS で 1378秒 → 189秒（7.3倍）。`skip_on_cran()` は一つも増やしていない。**
macOS の全テスト（NOT_CRAN=true）でも 293秒 → 207秒。
...
**2026-08-19 追記・訂正**: 修正前は 1378秒 対 win-builder 1315秒でほぼ一致したので「Windows の
代役が手に入った」と書いたが，**言い過ぎだった**。修正後は **189秒 対 422秒で2.2倍ずれる**。
一致していたのは **BLAS 律速の部分が支配的だったあいだだけ**で，それを取り除くと残りは
Windows 固有の遅さ（CPU・I/O）が効く領域になり比例しなくなる。

**使いどころ: どのテストが BLAS で重いかの特定と，重い順の順位づけ。絶対時間の予測には
使えない。**最終判断は win-builder に投げるしかない。

### その他

- **Title を `Test Data Engineering` へ**（issue #32・close 済）。CRAN incoming の書式検査も通過
- **`NEWS.md` に Removals 節**を新設（削除した関数とフィールドの対応表）
- **`cran-comments.md` を 2.0.0 用に全面改稿**。書く前に裏を取って**2件の誤りを訂正**した:
  非推奨警告は 1.9.0 からではなく**関数名 1.7.0 / フィールド名 1.8.0** から，
  ggExametrika は Imports ではなく **Suggests**
- **`Biclustering.Rd` の綴り2件を修正**（`wheter`→`whether`, `iterasions`→`iterations`）。
  `spell_check()` が拾った。**NEWS.md の履歴部分の誤字9件はそのまま残す**——あれは
...
- **`develop/myBiber.bib` が Newton 専用の絶対パス symlink だった**
  （`/Users/newton/Dropbox/myBiber.bib`）。Pythagoras ではリンク切れで `R CMD build` が
  落ちる。**相対パス（`../../../myBiber.bib`）に張り替えて両機で解決可能にした。**
  `.Rbuildignore` に `^develop$` があっても落ちるのは，**`R CMD build` が先に全体を
  一時ディレクトリへコピーしてから除外を適用する**ため
...
- rhub と win-devel の結果を待ち，**`cran-comments.md` の Test environments を実測へ書き換える**
...
殿から2件の指摘。(1) Array プロットで真っ白になる受験者がいる，(2) `FieldMembership` /
`FieldEstimated` に項目名が反映されない。どちらも実在のバグだった。
...
- **`ClassEstimated` は全モデルで無名**——二値でも。`SmoothedMembership` も同様
- `U`/`Q`/`Z` は行が無名(`dataFormat()` は列にだけ `ItemLabel` を貼っていた)
- `IRT$ability` は行が連番

**構造の問題は「明示的に貼っている場所が1箇所しかない」こと。**`rownames(StudentRank) <- tmp$ID`
だけが手書きで，そこから派生する `ClassEstimated` には何もしていない。他は**元の行列が
dimnames を持ち回っていれば残る偶然**で，だから**モデルごとに当たり外れが違った**。

### 対処: `R/00_labels.R` に集約

`label_result()` を各コンストラクタの `return` 直前で1回呼ぶ(18ファイル)。
`dataFormat()` も `U`/`Q`/`Z` の行にIDを貼る。

**同梱データは古い .rda なので `inherits(data,"exametrika")` の素通り経路にも貼る必要がある。**
ここを忘れると `J15S500` を直接渡した場合だけ無名のままになる。

**`FieldAnalysis` は対象外**。CRR とフィールドで並べ替えてあるので位置で貼ると別の項目の
名前が付く。結果は 20件 → 2件(CTTは `IfDeleted` 列にラベルを持つ・`ItemStatistics$ItemLabel`
はラベルそのもの)。数値は変わらない。

### Array プロット: 白潰れの正体は「1画素より細い枠は引けない」こと

セルごとに `rect(border="white", lwd=0.1)` で描いていた。**`lwd` をいくら小さくしても
デバイスは1画素未満の線を引けない**ので，行が画素より細くなると上下の白枠が塗りを覆う。
どの行が消えるかは**セル境界と画素格子の噛み合わせ**で決まるのでまだらになる
(「白い子がいる」という症状の説明)。

実測(全黒・400x600): 平均輝度 50行 0.01 / 200行 0.04 / 500行 0.19 / **821行 0.29**。
枠を消せばどの行数でも 0.00。J12S5000(5000行)を700px高で描くと 0.801 対 0.613
(真の黒率0.562)＝**インクの3〜4割が失われていた**。

`rasterImage()` へ置き換え。描画は 0.83秒 → 0.004秒(1パネル)。格子線はセルが6画素以上の
ときだけ引き，`nrows+ncols` 本の線で済ませる。

### 多値カラー版の検証で見つけた第2の罠(縮小時の平均)

**`rasterImage()` は行数が出力画素数を超えると平均して潰す。`interpolate=FALSE` は
...
- **testthat は既定で10件失敗すると打ち切る。**`Maximum number of 10 failures reached,
  some test results may be missing.` が出ているログの件数を全数と読んではいけない。
  全数を知るには `TESTTHAT_MAX_FAILS=1000`。
- **`devtools::test()` は自分で `NOT_CRAN=true` を立てる。**`env -u NOT_CRAN` を付けても
  無効。CRAN 構成を測るなら `testthat::test_file()` 側で `Sys.setenv(NOT_CRAN="false")`。
- **quartz の PNG デバイスはカラープロファイルを適用して書き出す。**`#E69F00` はファイル上
  では `#D47C0A` になる。画素値を指定色と直に突き合わせると「混色が出ている」と誤診する。
  検証するなら**同じデバイスで純色を描いて対応表を作ってから**比べる(実際に一度誤診した)。
- **親プロセスの CPU 0% を「止まった」と読むな。**テストの全数実行を2回 kill した。
  子のワーカは66%で回っていた。`top` で子まで見ること。
- **roxygen は `#'` を直前のブロックの続きとして読む。**関数本体の中の入れ子ヘルパに
  `#'` を付けたため `@noRd must not be followed by any text` が2件出た。本体内は `#`。

### 検証と成果物

全数(`NOT_CRAN=true`) **43ファイル・3,527件成功・失敗0・スキップ0**。落ちた13件は
いずれも「名前が付いたことによる形の不一致」で値の不一致はゼロ，`unname()` で対処。
新設 `tests/testthat/test-labels.R` は20件・CRAN構成で0.34秒。
コミット `3e9855f` / `a1efce5` / `bbfec43` / `d1ff4c9`，push 済み。

### 派生: AIC が正になる件(調査のみ・修正なし)

殿から「バイクラスタリングの AIC が正の数になっている」との指摘。**バグではない。**
二値・順序・評定は $\mathrm{AIC} = \chi^2 - 2\mathrm{df}$ なので，**χ² が 2df を超えれば正**。
名義だけは飽和モデルを作れないため $-2\log L + k$ に切り替わり，**常に正**(χ² と df は NA)。
順序は手元の4条件すべてで負になり，`AIC == chi_sq - 2*df` の一致も確認した。
**紛らわしいのは事実**なので，`print()` に定義を1行添える案を出したが，今回は見送り。

### 次

- 2.1.0 の本命は**多値BNM(StepReg/StepBNM・case C)**。
- CRAN 提出は**9月20日以降**(受理から1ヶ月)。`R CMD check --as-cran` と win-builder は未実施。
- 見送った案: 適合度指標の定義を `print()` に明示する / `criterion_scale` 属性で異なる定義の
  比較に警告を出す。

## 2026-09-06 日心のポスターを機に isotonic 系のソースを精査（コード変更なし）

日本心理学会第90回大会（9/4-6）のポスター発表で「順序制約を仮定して解いているのに，必ずしも達成できないのはなぜか」という質問を受け，原因を特定するためにソースを読んだ。**この日はコードを触っていない**。大会全体の記録は `~/Dropbox/03_対外活動/日本心理学会/第90回大会2026/聴講メモ.md`。

### 原因（確定）＝制約をかけている量と，SOAC/WOAC を判定している量が違う

**制約自体は効いている。**

- 制約＝`R/16_Biclustering_ordinal.R:293` の `iso_dual_map(Mcount, tol = 1e-6)`。El Barmi & Dykstra (1994) の Fenchel 双対座標上昇で，各フィールドの上側累積確率がランク方向に単調非減少になるよう制約付き MAP を解く。`viol_tol = 1e-6` で**原始実行可能性まで確認してから停止**するので，`model_esp = Σ q·BCRM` から作る **FRP の違反 `FRPmic` は 0 になる**
- ところが **`TRP` はモデル母数ではなく `BFRP1` から計算されている**（`:454-458`, `:466`）。
  `BFRP1[f,c] = Σ_q q·N_fcq·P_fcq / Σ_q N_fcq·P_fcq`，`N_fcq = t(fldmemb) %*% t(ZU[,,q]) %*% clsmemb`（事後重みつき観測度数）＝**データ由来で制約の対象外**
- 判定が入れ子（`:480-485`）＝`if (TRPmic == 0) { WOAC <- TRUE; if (FRPmic == 0) SOAC <- TRUE }`。**WOACが偽ならSOACは自動的に偽**。よって**全フィールドのFRPが完全に単調でも，経験的TRPが一箇所凹むだけで SOAC は FALSE** になる
- **傍証＝二値版は同じ問題を持たない**。`R/07_Biclustering.R:522` は `TRP <- colSums(PiFR * flddist)` ＝モデル母数 `PiFR` にクラス方向で一定の重みを掛けた和なので，FRP が単調なら TRP も必ず単調。**多値版だけが別定義**。既知の罠「ncls を増やすと SOAC が簡単に FALSE」も，`BFRP1` の度数が薄くなってノイズが乗るためで整合する

### 対処方針（未着手）

- **SOAC/WOAC の判定をモデル母数側へ揃える**（TRP を `model_esp` から作る）。isotonic 指定時は定義上つねに SOAC が真になり，制約が効いていることが出力に表れる
- 経験的 TRP は `TRP_empirical` として併記する。**モデルとデータのずれの指標**として使えるので捨てない

### ついでに見つかった要確認事項

1. **`BFRP1` の重みづけが二重ではないか**。事後期待観測得点なら `Σ q·N / Σ N` のはずだが `P_fcq` を掛けている。素の経験平均は `BFRP2`（`mean(Z*Q)`）が別に持つ。何を意図した量かの確認が要る
2. **`method="B"`（既定）だと `estimation="isotonic"` が黙って無視される**（`:69` の `use_isotonic <- (model == 2) && (estimation == "isotonic")`）。仕様としては正しい（Biclustering のクラスは順序なし）が**メッセージも警告も出ない**。既定が `"B"` なので事故りうる → 警告を出すべき
3. **収束時のロールバックが片方だけ**。`:354` で `BCRM <- oldBCRM` するが **`BBRM` は新しいまま**。以後 `testell` は `BCRM`，`nparam` は `BBRM` から計算されるので**1反復ずれた組み合わせ**になる。`R/07_Biclustering.R` も同型
4. **ソルバの反復上限のコメントが実装と食い違う**。コメントは「ソルバは自前の上限（既定100）を持つ」だが，R 側ラッパ `iso_dual_map()` の既定が `maxiter = 1000` で，呼び出しが `iso_dual_map(Mcount, tol = 1e-6)` なので **C++ の既定100は使われず1000が渡る**。安全側なので実害はないが記述が実装と違う

### 機能追加要望（ポスター来場者から）

**「PiFR の推定が終わったあと，回答者のランク分けをするだけの機能（予測）が欲しい」**

- 現状 `predict` は NAMESPACE に一つも無い。実体は**母数を固定した1回のEステップ**で，IRT でいう項目母数固定のスコアリングと同じ。返り値に `FRP`(=BCRM) と `FieldMembership` があるので**部品は揃っている**
- API 案＝`predict(object, newdata, type = c("class","posterior","score"))`。返り値は `Students` と同じ形。同じ仕組みで Biclustering / Ranklustering の全データ型と LCA・LRA まで対応できる
- **罠4つ**＝(1)**★`remap_category_codes()` の対応表が保存されていない**。新データに出現しないカテゴリがあるとコードがずれる。**適合時のマッピングをオブジェクトに保存する必要があり，返り値の構造変更を伴うのでやるなら早い方が得** (2)項目の同一性（2.1.0 で `R/00_labels.R` にラベルを集約したので時期は良い） (3)欠測 `Z` の扱いを適合時と揃える (4)Ranklustering の GTM 経路で予測時にフィルタ平滑化を適用するか（isotonic 経路には無いので曖昧さなし。既定は生の事後確率が無難）
- **副産物が大きい**＝これがあると**交差検証ができる**。学習データで推定→検証データで対数尤度＝**予測に基づくモデル選択**。A3 の「格子探索は真の(C,F)を当てられない・規準面が単峰でない」に対して**AIC/BIC とは別の規準**を持ち込める可能性がある

### 次

- 上の対処方針（SOAC 判定の統一）を最初に入れ，既存テストが通るかを見る。要確認事項2〜4もまとめて直す
- `predict()` は 2.1.0 の機能追加候補。**罠1（カテゴリマッピングの保存）は返り値の構造変更を伴うので早い方が得**
- 2.1.0 の本命が多値BNM(case C)，CRAN 提出が 9/20 以降という方針は変更なし

## 2026-09-10 リリース計画を確定（2.0.1 → 2.1.0 に統合）

9/6のisotonic精査で見つけた不具合のうち**推定値が変わるものがある**ため，パッチ版として用意していた 2.0.1 を **2.1.0 に格上げして1本にまとめる**方針とした。NEWSの「no change to any estimate」は撤回して書き直す。

**判断の根拠**: NEWSは自分で書いた文書であり，文言に合わせてバグの修正を先送りするのは順序が逆。版番号を上げてNEWSを直せば済む。
...
**影響範囲は思ったより狭い。**`nparam` が `BBRM` を使うのは **`use_isotonic` の分岐だけ**。他の分岐（`ncls*nfld*(maxQ-1)` 等）は構造から決まるので影響しない。よって

> **多値バイクラ ＋ `method="R"` ＋ `estimation="isotonic"` のときの `nparam`／AIC／BIC／df**

だけが対象。ただし**この分岐は例外パスではない**——`test_log_lik - old_test_log_lik <= 0` は尤度が改善しなくなったとき，つまり**通常の収束経路**で，tolによる別の収束判定はない。**ほぼ毎回通る**。

**★ 2026-09-06 の記述を訂正**: 「`R/07_Biclustering.R` も同型」は**誤り**。二値版は `PiFR <- oldPiFR`（`:503`）で戻し，`nparam` も `PiFR` から作る（`:581`）ので**戻す変数と使う変数が一致しており，ずれない**。**多値版だけの問題**。

**修正自体は1行**（`BBRM` も戻すか，`BCRM` から `:151` の漸化式で再構成）。**重いのは検証**——推定値が変わるので既存テストの期待値と Mathematica 由来の参照フィクスチャとの突合が要る。
...
| **2.2.0** | **SOAC/WOAC の判定をモデル母数側（`model_esp`）へ揃える**（経験的TRPは `TRP_empirical` として併記）＋ **`predict()`**（母数固定の1回Eステップ。罠＝`remap_category_codes()` の対応表が未保存。副産物として交差検証によるモデル選択の道が開く） | 年内 |
| **2.3.0** | **多値BNM（StepReg / case C）** | 年明け |

**SOAC/WOACの判定変更を2.1.0に入れない理由**: `TRP_empirical` の併記は**出力仕様の変更**で設計判断を含む。急ぐと雑になる。`predict()` も返り値の構造変更を伴うので，まとめて2.2.0で扱う。

### 提出の段取り

- CRAN再提出は **2.0.0 の受理（2026-08-20）から1ヶ月＝9/20以降**。ただし**9/20は下限であって締切ではない**
...
ベイズ塾合宿（9/19 の研究発表）の資料づくりで LRA の GTM と isotonic を比べていて，
SOM だけが毎回「収束しなかったかもしれない」と警告を出すことに気づいたのが発端。
`R/00_EMclus.R` の `somclus()` を Mathematica 原典（`develop/mtmk15forVer13/mod/Module_LRA.wl`）
と読み合わせ，移植の過程で入った食い違いを 5 件直した。

### 原典との照合で分かったこと

本家の SOM は `maxT = 100` を走り切って黙って終わる。収束判定は存在しない。
提示順は `SeedRandom[Total[uuu] + somt]` で**エポックごとに種を引き直す**ので，
データが同じなら結果は決まる（決定的）。R 版の `seed = NULL` 経路はこれを正確に
再現していた。

- **seed の凍結（R 独自のバグ）**: `seed` を渡すと `set.seed(seed)` をループ内で毎回
  呼ぶため，提示順が全エポックで同一になっていた。本家に `seed` 引数は無い。
  `seed_base + somt` へ変更。`seed = NULL` の数値は従来と同一。
- **収束報告（R 独自の設計ミス）**: 判定が無いこと自体は本家どおりだが，R は完走を
  `converge = FALSE` ＋警告で報告していた。既定経路の警告を廃止。
- **`mic` の適用位置**: 本家は学生 1 人の更新直後に `Sort /@ refmat`。R はエポック末に
  まとめて掛けており，勝者の決定に使う行列が別物になる＝別アルゴリズム。`conf` も
  同じ位置へ揃えた（`conf` は本家に無い R 独自の機能）。
- **タイブレーク**: 本家 `Sort[...][[-1]]` は同値なら大きいランク。`which.max` は小さい
  ランクを返していたので向きを合わせた。
- **`.Random.seed` の破壊**: 毎反復で `set.seed` を呼びながら戻していなかったため，
  呼び出し側の乱数列を壊していた。`on.exit` で復帰。
- **丸めは合わせない**: 本家は `hhhmat` を `10^-6` で丸めるが，影響が軽微なので対応しない。
- **`maxiter` の既定**: 本家は `maxT = 100` 固定。R は `LRA.binary` が 1000 を渡す。
  **1000 のまま据え置き**と決めた。SOM では `maxiter` が焼きなましのスケジュールそのもの
  （`alpha` 1→0.01，`sigma` 1→0.12 を maxiter ステップで線形に落とす）なので，
  増やすと「長く回す」ではなく「冷まし方を変える」ことになり，解が変わる。

### Mathematica と数値を突合しない理由（結論）

**SOM は乱数エンジンに依存するので，本家と同じ種を与えても提示順の系列が違い，一致しない。**
`tools/regen_fixtures.sh` に SOM（method = 1）の行が無いのはそのため。突合したくなったら，
Mathematica 側から提示順 `is` を書き出して R に読ませれば決定的な比較はできるが，
荘島先生が SOM を推奨していないので割に合わない。代わりにテスト内へ独立に書いた
参照実装（`som_reference()`）との一致でアルゴリズムを固定した。

### C++ 化

`src/som_core.cpp` を新設し，1 エポック分の学生ループ（勝者判定・近傍更新・`conf`・`mic`・
事前分布）を移した。**提示順の生成と外側ループは R に残す**。理由は (a) `set.seed` の
再現性を保つため，(b) `BIC.check` の早期停止が `ItemFit()` を呼ぶため。
...
`mic = TRUE` を学生ごとに直すと素朴な `t(apply(M, 1, sort))` では 20 倍遅くなる（50 万回の
並べ替え）。奇偶転置ソートで 7.1 秒まで落としたが，C++ 化で不要になったので削除した。
残る 1.9 秒はほぼ `log()` の呼び出し回数（学生 1 人あたり 180 回 × 50 万人 = 9000 万回）で
決まっており，アルゴリズムを変えない限りこれ以上は詰まらない。
`postdist` を毎エポックではなくスケジュール終了後に 1 回だけ計算するようにも直した。

### テスト

`tests/testthat/test-lra-som.R`（新設・5 本）。SOM のテストはこれまで 1 本も無かった。
**うち 1 本はテスト内に独立に書いた参照実装との数値一致**で，これだけが提示順の凍結と
`mic` の位置ずれを検出できる。変異テストで確認済み＝どちらを元に戻しても
`test-lra-som.R:87` が落ち，他の 4 本は通ってしまう。

### リリース

- `DESCRIPTION` を **2.1.0** へ繰り上げ。NEWS は **2.0.1 の節を原文のまま残し，2.1.0 を新設**
  （v2.0.1 のタグは GitHub に公開済みなので吸収しない）。
- 全テスト 3536 件 pass。`git archive HEAD` した木で `R CMD check --as-cran` は
  **0 errors / 0 warnings / 1 note**。note は 2.0.1 の Array ラスタ化由来で
  `importFrom("graphics", "grconvertX", "grconvertY", "rasterImage")` の追加を促すもの。**未対応**。
- コミット `085a2dd`（fix）と `25db7d8`（docs）を main へ push 済み。

### 次回への引き継ぎ

- **isotonic のロールバック片落ち修正（`16_Biclustering_ordinal.R:353`）は未着手**。2.1.0 の
  繰り上げのそもそもの理由がこれなので，提出前に必ず入れる。
