...
![](../man/figures/exametrika.png){style="float: right; width: 150px; margin-left: 20px;"}
...
- J15S3810: 順序尺度データセット(15 項目 4件法, 3810人の受験者)
  - 順序尺度版潜在ランクモデルに用います
- J35S500: 順序尺度多値データセット（35項目 5件法、500人の受験者）
  - 順序尺度バイクラスタリングの例で使用
- J20S600: 名義尺度多値データセット（20項目 4カテゴリ、600人の受験者）
  - 名義尺度バイクラスタリングの例で使用
...
グリッドサーチ関数は、統計的適合度指標に基づいて最適なクラス数とフィールド数の体系的な探索を行います。このアプローチは複数のパラメータ組み合わせを評価し、AIC、BIC、またはその他の適合度指標などの指定された基準に従って最適適合モデルを選択します。
...
無限関係モデルはチャイニーズレストランプロセスを使用して、最適なフィールド数とクラス数を自動的に探索します。このモデルは、事前に適切な潜在構造数がわからない場合に特に有用です。

**重要な注意点：**
- 特に大きなデータセットでは実行に時間がかかることがあります
- モデルは大きめの数のクラスとフィールドを検出する傾向があります
- 計算資源を考慮して注意深く使用してください
...
バイクラスタリングとランククラスタリングは、順序尺度や名義尺度の多段階反応データに対しても実行することができます。Arrayプロットでは欠測値が黒で表示されます。カラーパレットは`colors`オプションを使用して異なる色彩スキームを指定することでカスタマイズできます。
...
順序尺度の多値バイクラスタリングでは、いくつかの新しいプロットタイプが利用可能です。FRP（フィールド参照プロファイル）は、各フィールドの潜在ランクごとの期待得点を折れ線グラフで表示します。`stat`パラメータで要約統計量を指定できます："mean"（デフォルト）、"median"、"mode"。

```{r bic_poly_ord_frp, fig.width=7, fig.height=5}
plot(result.B.ord, type = "FRP", nc = 3, nr = 2)
```

FCRP（フィールドカテゴリ反応プロファイル）は、各反応カテゴリの確率を潜在ランクごとに表示します。`style`パラメータで"line"（デフォルト、折れ線）または"bar"（積み上げ棒グラフ）を選択できます。
...
FCBR（フィールド累積境界参照）は、各フィールドの累積境界確率 P(Q >= q) を表示します。このプロットタイプは順序尺度データでのみ利用可能です。

```{r bic_poly_ord_fcbr, fig.width=7, fig.height=5}
plot(result.B.ord, type = "FCBR", nc = 3, nr = 2)
```

ScoreFieldプロットは、フィールドと潜在ランクにわたる期待得点をヒートマップで表示します。

```{r bic_poly_ord_scorefield, fig.width=7, fig.height=5}
plot(result.B.ord, type = "ScoreField")
```

RRV（ランク参照ベクトル）は、FRPの転置版で、フィールドをx軸に、各潜在ランクの折れ線を重ねて比較できます。
...
名義尺度の多値バイクラスタリングでも、FRP、FCRP、ScoreField、RRVプロットが利用可能です。ただし、境界確率は順序カテゴリでのみ意味を持つため、FCBRは名義尺度データでは利用できません。
