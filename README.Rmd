...
A community-hosted preview page lets collaborators try an `exametrika`-style workflow in the browser before installing R — upload a response matrix (CSV/Excel), pick an analysis goal (IRT / latent rank / learning route / biclustering / Test Data Engineering report), and get an illustrative output. Useful for showing result mock-ups to colleagues or seminar students.
...
Not affiliated with the package authors. See [#29](https://github.com/kosugitti/exametrika/issues/29) for context.
