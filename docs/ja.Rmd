...
- 欠測値
 - NA値がサポートされています
 - `dataFormat()`を使用して欠測値を適切に処理
 - 空文字列や特殊な欠測値コードは避ける

- データ構造
 - 反応データは数値でなければなりません
 - 2値データは0/1である必要があります
 - 多値データは順序付き整数である必要があります

### ネットワーク構造の指定

LDLRA、LDB、BINETを使用する際のネットワーク指定に関する一般的な問題：

- ファイル形式
 - CSVファイルにはヘッダーが必要です
 - 列名は期待される形式と一致する必要があります
 - 余分な空白や非表示文字がないか確認

- グラフ構造
 - 非循環（DAG）でなければなりません
 - 親子関係は論理的な順序を尊重する必要があります
 - ノードあたりの親の数は適切な範囲である必要があります

### 収束の問題

反復手続き（IRT、LCA、LRA）について：
- 異なる初期値を試してみる
- 最大反復回数を調整する
- モデルの複雑さを減らすことを検討する
...
1. データの検証
  - `str()`でデータ構造を必ず確認
  - `summary()`で予期しない値をチェック
  - `TestStatistics()`で反応パターンを確認
...
- [GitHub Issues](https://github.com/kosugitti/exametrika/issues)に課題を開く
- 最小限の再現可能な例を提供する
- Rのセッション情報（`sessionInfo()`）を含める
...
[GitHub Discussions](https://github.com/kosugitti/exametrika/discussions)ページで参加できます。
